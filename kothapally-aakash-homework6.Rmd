...
```{r fig:trace-z1, echo=FALSE, fig.width=7,fig.height=6,fig.cap="\\label{fig:trace-z}Traceplot of $z_3, z_8, z_9, z_{10}, z_{12}.$"}
par(mfrow=c(2,3))
...
```{r fig:run-z1, echo=FALSE, fig.width=7,fig.height=6,fig.cap="\\label{fig:run-z}Running average plots of $z_3, z_8, z_9, z_{10}, z_{12}.$"}
par(mfrow=c(2,3))
plot(1:n.iter, run.avg[,1], type = "l",
     xlab = "Iteration", ylab = expression(theta),
     main = expression(paste("Running Average Plot of ", theta)))
...
We can see for theta and censored values, the running average converges onto a value for all of them. As a result, I do believe these diagnostic plots suggest we have run the sampler long enough.
...
2d. We can consider the posterior densities when altering the r, a, and b values.
...
```{r fig:density-theta1, echo=FALSE, fig.width=7,fig.height=6,fig.cap="\\label{fig:density-theta}Estimated posterior density of theta"}
par(mfrow=c(2,1))
# density plots
plot(density(res[,1]), xlab = expression(theta), 
